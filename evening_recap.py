...
def is_valid_ticker(t):
    ...
    if t in _KNOWN_ETFS: return False
    return True
...
def main():
    ...
    md   = render_markdown(target, movers, near_miss)
    html_out = render_html(target, movers, near_miss)

    md_file   = f"recap_{date_str}.md"
    html_file = f"recap_{date_str}.html"
    with open(md_file,   "w", encoding="utf-8") as f: f.write(md)
    with open(html_file, "w", encoding="utf-8") as f: f.write(html_out)
